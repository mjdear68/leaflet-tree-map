...
m <- leaflet(elementId = "Trees", options = leafletOptions(minZoom = 15, maxZoom = 20)) |>
  addTiles() |> # Gives Open Street Map tiles
  setView(150.2864, -33.6268, zoom = 18) |>
  addCircleMarkers(data = df_map, popup = ~htmlEscape(paste("Girth: ", Girth_mm, "mm", sep = "")), radius= df_map$Girth_mm/max(df_map$Girth_mm)*5, color = ~pal(Girth_mm))
